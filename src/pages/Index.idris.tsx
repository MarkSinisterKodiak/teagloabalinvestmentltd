...
import { Truck, Package, Clock, Shield, MapPin, MessageCircle, Menu, X, Phone, Mail, ChevronRight, CheckCircle2, Globe, ArrowRight, Briefcase, Building2, Users, Route, Star, Award, FileCheck, MapPinned, Warehouse, ChevronDown, HelpCircle, Zap } from "lucide-react";
...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 safe-top ${scrolled ? "bg-white/95 backdrop-blur-xl shadow-md border-b border-gray-100" : "bg-transparent"}`}>
      {/* Top info bar — desktop only */}
      <div className={`hidden lg:block text-xs transition-all duration-300 overflow-hidden ${scrolled ? "max-h-0 opacity-0" : "max-h-10 opacity-100 bg-gray-900/80 backdrop-blur-sm text-gray-300"}`}>
      ...
      <div className="container mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 xs:h-16 md:h-20">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <button
                ...
                className={`font-medium transition-colors px-3 py-1.5 rounded-full text-sm ${scrolled ? "text-gray-600 hover:text-blue-700 hover:bg-blue-50" : "text-white/90 hover:text-white hover:bg-white/10"}`}
              >
                {link.label}
              </button>
            ))}
          ...
          <div className="hidden md:flex items-center gap-3">
            <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 px-3 py-2 rounded-full font-medium text-sm transition-colors ${scrolled ? "bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20" : "bg-white/10 text-white hover:bg-white/20"}`}>
              <MessageCircle className="w-4 h-4" />
              WhatsApp
            </a>
            <Button size="sm" onClick={() => scrollToSection("quote")} className="bg-blue-600 hover:bg-blue-700 text-white shadow-md rounded-full px-5">
              Get a Quote
            </Button>
          </div>

          {/* Mobile menu toggle */}
          <button className={`lg:hidden p-2 rounded-lg transition-colors ${scrolled ? "hover:bg-gray-100 text-gray-800" : "hover:bg-white/10 text-white"}`} onClick={() => setIsOpen(!isOpen)}>
        ...
        {/* Mobile menu */}
        {isOpen && (
          <div className="lg:hidden py-4 border-t border-gray-200 bg-white rounded-b-2xl shadow-xl animate-in slide-in-from-top duration-200">
...
/* ─── HERO ─── */
const HeroSection = () => (
  <section className="relative min-h-[100svh] flex items-center overflow-hidden landscape-min-h-auto">
    <div className="absolute inset-0 z-0">
      <img src={ukTruckHero} alt="" className="absolute inset-0 w-full h-full object-cover scale-105" />
      <div className="absolute inset-0 bg-gradient-to-br from-gray-900/90 via-blue-950/80 to-gray-900/85" />
      <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-white to-transparent" />
    </div>

    <div className="container mx-auto px-3 xs:px-4 sm:px-6 lg:px-8 relative z-10 py-24 xs:py-28 sm:py-32 md:py-36">
      <div className="grid lg:grid-cols-12 gap-6 xs:gap-8 lg:gap-12 items-center">
        {/* Content */}
        <div className="lg:col-span-7 text-center lg:text-left">
          <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 xs:py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 mb-4 xs:mb-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
          ...
          <h1 className="font-display text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold mb-3 xs:mb-4 sm:mb-6 leading-[1.08] text-white animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150">
            Professional<span className="block bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">Freight Transport</span>Across the UK
          </h1>

          <p className="text-sm xs:text-base sm:text-lg text-gray-300 mb-4 xs:mb-6 sm:mb-8 max-w-xl mx-auto lg:mx-0 leading-relaxed animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
          ...
          {/* Trust Badges */}
          <div className="flex flex-wrap justify-center lg:justify-start gap-2 xs:gap-3 mb-4 xs:mb-6 sm:mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-500">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-green-500/15 border border-green-500/25 backdrop-blur-sm">
              <CheckCircle2 className="w-3.5 h-3.5 text-green-400" />
              <span className="text-[10px] xs:text-xs sm:text-sm text-green-300 font-medium">Companies House Registered</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-500/15 border border-blue-500/25 backdrop-blur-sm">
              <Shield className="w-3.5 h-3.5 text-blue-400" />
              <span className="text-[10px] xs:text-xs sm:text-sm text-blue-300 font-medium">Goods in Transit Insured</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-500/15 border border-amber-500/25 backdrop-blur-sm">
              <Award className="w-3.5 h-3.5 text-amber-400" />
              <span className="text-[10px] xs:text-xs sm:text-sm text-amber-300 font-medium">GPS Tracked Fleet</span>
            </div>
          </div>

          <div className="flex flex-col xs:flex-row gap-2 xs:gap-3 sm:gap-4 mb-6 xs:mb-8 sm:mb-10 justify-center lg:justify-start w-full xs:w-auto animate-in fade-in slide-in-from-bottom-4 duration-700 delay-700">
            <Button size="lg" onClick={() => scrollToSection("quote")} className="group bg-blue-600 hover:bg-blue-700 text-white font-semibold h-11 xs:h-12 sm:h-14 px-4 xs:px-5 sm:px-6 md:px-8 text-xs xs:text-sm sm:text-base shadow-lg shadow-blue-600/30 hover:shadow-xl hover:shadow-blue-600/40 transition-all touch-target w-full xs:w-auto rounded-xl">
            ...
            <a href={`tel:${company.whatsappNumber}`} className="inline-flex items-center justify-center gap-2 border-2 border-white/30 text-white hover:bg-white/10 font-semibold h-11 xs:h-12 sm:h-14 px-4 xs:px-5 sm:px-6 md:px-8 text-xs xs:text-sm sm:text-base touch-target w-full xs:w-auto rounded-xl transition-colors backdrop-blur-sm">
          ...
          {/* Stats */}
          <div className="grid grid-cols-3 gap-4 xs:gap-6 sm:gap-8 max-w-lg mx-auto lg:mx-0 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-1000">
            <div className="text-center lg:text-left">
              <div className="text-2xl xs:text-3xl sm:text-4xl font-display font-bold text-white">3<span className="bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">+</span></div>
              <div className="text-[10px] xs:text-xs sm:text-sm text-gray-400 mt-1">Years Operating</div>
            </div>
            <div className="text-center lg:text-left">
              <div className="text-2xl xs:text-3xl sm:text-4xl font-display font-bold text-white">500<span className="bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">+</span></div>
              <div className="text-[10px] xs:text-xs sm:text-sm text-gray-400 mt-1">Loads Delivered</div>
            </div>
            <div className="text-center lg:text-left">
              <div className="text-2xl xs:text-3xl sm:text-4xl font-display font-bold text-white">24<span className="bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">/7</span></div>
        ...
        <div className="lg:col-span-5 hidden lg:block animate-in fade-in slide-in-from-right-8 duration-700 delay-500">
          <div className="bg-white rounded-2xl shadow-2xl shadow-black/20 overflow-hidden ring-1 ring-white/10">
            <div className="bg-gradient-to-r from-gray-900 to-gray-800 px-6 py-5">
              <div className="flex items-center justify-between">
                ...
                <div className="w-14 h-14 bg-blue-600 rounded-xl flex items-center justify-center shadow-lg animate-pulse-scale">
                  <Phone className="w-7 h-7 text-white" />
                </div>
              </div>
            </div>
            <div className="p-5 space-y-2 bg-gray-50">
              <a href={`https://wa.me/${company.whatsappNumber.replace(/[^0-9]/g, "")}`} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-4 p-3 bg-white hover:bg-green-50 rounded-xl border border-gray-100 hover:border-green-200 transition-all hover:shadow-md">
                <div className="w-10 h-10 bg-green-500 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm"><MessageCircle className="w-5 h-5 text-white" /></div>
              ...
              <a href={`mailto:${company.email}`} rel="noopener noreferrer" className="group flex items-center gap-4 p-3 bg-white hover:bg-blue-50 rounded-xl border border-gray-100 hover:border-blue-200 transition-all hover:shadow-md">
                <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm"><Mail className="w-5 h-5 text-white" /></div>
            ...
            <div className="p-5 bg-white border-t border-gray-100">
              ...
              <p className="text-center text-xs text-gray-400 mt-2">No obligation &bull; Quoted within 2 hours</p>
            </div>
          </div>
        </div>
      </div>
    ...
    {/* Mobile Contact Bar */}
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-t border-gray-200 shadow-lg safe-bottom">
      <div className="flex items-center justify-around py-2.5 xs:py-3 px-2 xs:px-4">
        ...
        <button onClick={() => scrollToSection("quote")} className="bg-blue-600 text-white px-4 xs:px-6 py-2 xs:py-2.5 rounded-full font-semibold text-xs xs:text-sm shadow-md shadow-blue-600/30 touch-target">Get a Quote</button>
      </div>
    </div>
  </section>
);
...
/* ─── TRUST BAR ─── */
const TrustBar = () => {
  const fade = useFadeIn(0.2);
  return (
    <div ref={fade.ref} className={`bg-white border-b border-gray-100 py-6 sm:py-8 ${fade.className}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-10 md:gap-14">
          {[
            ...
            <div key={item.label} className="flex items-center gap-2 text-gray-500 group">
              <item.icon className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 group-hover:scale-110 transition-transform" />
...
const SectionDivider = () => {
  const loads = useCountUp(500, 2000);
  const clients = useCountUp(150, 2000);
  const onTime = useCountUp(99, 1800);
  const damage = useCountUp(0, 500);
  const stats = [
    { counter: loads, suffix: "+", label: "Loads Delivered", icon: Package },
    { counter: clients, suffix: "+", label: "Business Clients", icon: Users },
    { counter: onTime, suffix: "%", label: "On-Time Rate", icon: Clock },
    { counter: damage, suffix: "", label: "Damage Claims", icon: Shield },
  ];
  return (
    <div className="relative py-16 sm:py-20 overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {stats.map((stat) => (
            <div key={stat.label} ref={stat.counter.ref} className="text-center group">
              ...
              <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-1">{stat.counter.count}{stat.suffix}</div>
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <div ref={fade.ref} className={`relative ${fade.className}`}>
            <div className="relative rounded-2xl overflow-hidden shadow-2xl group">
              <img src={ukTruckFleet} alt="Idris Transport fleet" className="w-full h-[350px] sm:h-[450px] object-cover group-hover:scale-105 transition-transform duration-700" />
            ...
            <div className="absolute -top-4 -left-4 sm:left-8 bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl shadow-xl p-4 text-white hidden sm:block">
          ...
          <div ref={fadeRight.ref} className={fadeRight.className}>
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-6 leading-tight">
              London-Based<br /><span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Transport Specialists</span>
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
              {[
                ...
                <div key={item.title} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50 hover:bg-blue-50 transition-all hover:shadow-sm group cursor-default">
                  <div className="w-10 h-10 rounded-lg bg-blue-100 group-hover:bg-blue-200 flex items-center justify-center flex-shrink-0 transition-colors"><item.icon className="w-5 h-5 text-blue-700" /></div>
            ...
            <Button size="lg" onClick={() => scrollToSection("quote")} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold h-13 px-8 text-base shadow-lg shadow-blue-600/20 hover:shadow-xl transition-all rounded-xl group">
...
const HowItWorksSection = () => {
  const fade = useFadeIn();
  return (
    <section ref={fade.ref} className={`py-16 sm:py-20 bg-gray-50 border-y border-gray-100 ${fade.className}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">How It <span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Works</span></h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto relative">
          ...
          {[
            ...
            <div key={item.step} className="relative text-center group" style={{ transitionDelay: `${i * 150}ms` }}>
              <div className="w-20 h-20 rounded-2xl bg-white border-2 border-blue-200 group-hover:border-blue-500 group-hover:shadow-lg group-hover:shadow-blue-100 flex items-center justify-center mx-auto mb-6 transition-all duration-300 relative z-10 group-hover:-translate-y-1">
                <item.icon className="w-8 h-8 text-blue-600" />
                <div className="absolute -top-3 -right-3 w-8 h-8 bg-gradient-to-br from-blue-600 to-blue-700 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-md">{item.step}</div>
              </div>
...
const ServicesSection = () => {
  const fade = useFadeIn();
  return (
    <section id="services" ref={fade.ref} className={`py-16 sm:py-20 md:py-28 bg-white relative overflow-hidden ${fade.className}`}>
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-50 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl opacity-50 pointer-events-none" />
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-12 md:mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 text-gray-900">Our <span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Services</span></h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, i) => (
            <div key={service.title} className="group relative bg-white rounded-2xl p-6 sm:p-8 shadow-sm hover:shadow-xl border border-gray-100 hover:border-blue-200 transition-all duration-300 hover:-translate-y-2" style={{ transitionDelay: `${i * 75}ms` }}>
              ...
              <div className="relative z-10">
                <div className="w-14 h-14 rounded-2xl bg-blue-50 group-hover:bg-blue-600 flex items-center justify-center mb-6 transition-colors duration-300 group-hover:shadow-lg group-hover:shadow-blue-200"><service.icon className="w-7 h-7 text-blue-700 group-hover:text-white transition-colors" /></div>
...
const CoverageSection = () => {
  const fade = useFadeIn();
  const regions = [
    { name: "England", cities: ["London", "Manchester", "Birmingham", "Leeds", "Liverpool", "Bristol", "Sheffield", "Newcastle", "Nottingham", "Southampton", "Leicester", "Coventry"] },
    { name: "Scotland", cities: ["Edinburgh", "Glasgow"] },
    { name: "Wales & N. Ireland", cities: ["Cardiff", "Belfast"] },
  ];
  return (
    <section id="coverage" ref={fade.ref} className={`py-16 sm:py-20 bg-gray-50 border-t border-gray-100 ${fade.className}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 text-gray-900">Areas We <span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Cover</span></h2>
          ...
          {regions.map((region) => (
            <div key={region.name}>
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-blue-500" />{region.name}
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
                {region.cities.map((city) => (
                  <div key={city} className="flex items-center gap-2 px-4 py-3 bg-white rounded-xl border border-gray-100 hover:border-blue-300 hover:bg-blue-50 hover:shadow-sm transition-all group cursor-default">
                    <MapPin className="w-4 h-4 text-blue-500 flex-shrink-0 group-hover:scale-110 transition-transform" />
        ...
        <div className="text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-full mb-4">
            ...
            <span className="text-sm text-blue-700 font-medium">Don't see your area? We cover all UK postcodes — just ask.</span>
          </div>
          <br />
          <Button onClick={() => scrollToSection("quote")} className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl h-12 px-6 shadow-md shadow-blue-600/20 hover:shadow-lg transition-all">Check Your Route<ArrowRight className="w-4 h-4 ml-2" /></Button>
        </div>
      </div>
    </section>
  );
...
const TestimonialsSection = () => {
  const fade = useFadeIn();
  return (
    <section ref={fade.ref} className={`py-16 sm:py-20 bg-white ${fade.className}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-gray-900">What Our <span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Clients Say</span></h2>
          <p className="text-gray-500 max-w-xl mx-auto">Trusted by businesses across the UK for reliable, professional freight transport.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {[
            ...
            <div key={review.name} className="relative bg-gray-50 rounded-2xl p-6 sm:p-8 border border-gray-100 hover:shadow-lg hover:border-blue-100 transition-all duration-300 group" style={{ transitionDelay: `${i * 100}ms` }}>
              <div className="absolute top-4 right-6 text-6xl font-serif text-blue-100 leading-none group-hover:text-blue-200 transition-colors">&rdquo;</div>
              <div className="flex gap-1 mb-4">{[...Array(review.rating)].map((_, i) => (<Star key={i} className="w-4 h-4 text-amber-400 fill-amber-400" />))}</div>
              <p className="text-gray-600 text-sm leading-relaxed mb-6 relative z-10">&ldquo;{review.text}&rdquo;</p>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center shadow-sm"><span className="text-white font-bold text-sm">{review.name.charAt(0)}</span></div>
...
const FAQSection = () => {
  ...
  const fade = useFadeIn();
  const faqs = [
    { q: "What areas do you cover?", a: "We cover the entire United Kingdom — England, Scotland, Wales, and Northern Ireland. Whether it's a local London delivery or a long-haul run to Edinburgh, we've got you covered." },
    { q: "How quickly can you collect?", a: "We offer same-day collection for urgent loads (subject to availability). For standard bookings, we can usually collect within 24 hours of confirming your quote." },
    { q: "Are my goods insured during transport?", a: "Yes. Every load is covered by comprehensive Goods in Transit insurance and we also hold Public Liability insurance. We can provide certificates on request." },
    { q: "What types of freight do you carry?", a: "We handle palletised goods, loose cargo, construction materials, machinery, industrial supplies, and general commercial freight. If it fits on our vehicles, we can move it." },
    { q: "How do I get a quote?", a: "Simply fill out our online quote form, call us, or send a WhatsApp message. Tell us what you're shipping, where from, and where to — we'll respond with a competitive price within hours." },
    { q: "Do you provide proof of delivery?", a: "Yes. Every delivery includes a signed Proof of Delivery (POD), which is sent to you digitally for your records." },
  ];

  return (
    <section ref={fade.ref} className={`py-16 sm:py-20 bg-gray-50 border-t border-gray-100 ${fade.className}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-full mb-4"><HelpCircle className="w-4 h-4 text-blue-700" /><span className="text-blue-700 font-semibold text-sm">Common Questions</span></div>
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-gray-900">Frequently Asked <span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Questions</span></h2>
          </div>

          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <div key={i} className={`bg-white rounded-xl border transition-all duration-200 ${openIndex === i ? "border-blue-200 shadow-md" : "border-gray-100 hover:border-gray-200"}`}>
                <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full flex items-center justify-between px-5 sm:px-6 py-4 sm:py-5 text-left gap-4">
                  <span className="font-semibold text-gray-900 text-sm sm:text-base">{faq.q}</span>
                  <ChevronDown className={`w-5 h-5 text-gray-400 flex-shrink-0 transition-transform duration-200 ${openIndex === i ? "rotate-180 text-blue-600" : ""}`} />
                </button>
                <div className={`overflow-hidden transition-all duration-300 ${openIndex === i ? "max-h-60 opacity-100" : "max-h-0 opacity-0"}`}>
                  <div className="px-5 sm:px-6 pb-5 text-gray-600 text-sm leading-relaxed border-t border-gray-50 pt-4">{faq.a}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-center mt-8">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              <a href={`https://wa.me/${company.whatsappNumber.replace(/[^0-9]/g, "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 font-medium text-sm transition-colors">
                <MessageCircle className="w-4 h-4" />Ask on WhatsApp
              </a>
              <a href={`mailto:${company.email}`} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 text-blue-700 hover:bg-blue-100 font-medium text-sm transition-colors">
  ...
  <div className="relative py-20 sm:py-24 overflow-hidden">
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
      <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full mb-6 border border-white/20">
        ...
        <span className="text-white text-sm font-medium">Quoted within 2 hours — guaranteed</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        <Button size="lg" onClick={() => scrollToSection("quote")} className="bg-white text-blue-700 hover:bg-gray-100 font-bold h-14 px-8 text-base shadow-xl rounded-xl hover:shadow-2xl transition-all group">Get Your Free Quote<ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" /></Button>
        <a href={`https://wa.me/${company.whatsappNumber.replace(/[^0-9]/g, "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 h-14 px-8 rounded-xl border-2 border-white/30 text-white font-bold text-base hover:bg-white/10 transition-colors backdrop-blur-sm">
...
const RequestQuoteSection = () => {
  ...
  return (
    <section id="quote" ref={fade.ref} className={`py-12 xs:py-16 sm:py-20 md:py-24 bg-gradient-to-br from-gray-50 via-white to-blue-50/30 relative overflow-hidden ${fade.className}`}>
      ...
      <div className="container mx-auto px-3 xs:px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-8 xs:mb-10 md:mb-14">
            ...
            <h2 className="font-display text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold mb-3 xs:mb-4 text-gray-900">Get Your <span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Quote Today</span></h2>
            ...
            <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 px-4 xs:px-6 sm:px-8 py-4 xs:py-5 sm:py-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div className="flex items-center gap-2 xs:gap-3">
                  <div className="flex items-center gap-1.5 px-2.5 xs:px-3 py-1 xs:py-1.5 bg-white/10 rounded-full"><Clock className="w-3 h-3 xs:w-3.5 xs:h-3.5 text-blue-400" /><span className="text-white text-[10px] xs:text-xs font-medium">2hr Response</span></div>
            ...
            <div className="p-4 xs:p-6 sm:p-8 md:p-10">
              {status === "success" ? (
                <div className="text-center py-8 xs:py-12">
                  <div className="w-16 h-16 xs:w-20 xs:h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4 xs:mb-6 ring-4 ring-green-50 animate-in zoom-in duration-300"><CheckCircle2 className="w-8 h-8 xs:w-10 xs:h-10 text-green-600" /></div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <div className="flex items-center gap-2 mb-4"><div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center"><span className="text-white font-bold text-xs">1</span></div><h4 className="font-semibold text-gray-900 text-sm xs:text-base">Your Details</h4></div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 xs:gap-4">
                      <div className="space-y-1.5"><label className="text-xs xs:text-sm font-medium text-gray-700">Full Name *</label><input type="text" name="fullName" value={formData.fullName} onChange={handleChange} placeholder="John Smith" required className="w-full h-10 xs:h-11 sm:h-12 px-3 xs:px-4 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition-all text-sm sm:text-base" /></div>
                      <div className="space-y-1.5"><label className="text-xs xs:text-sm font-medium text-gray-700">Email *</label><input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="john@company.co.uk" required className="w-full h-10 xs:h-11 sm:h-12 px-3 xs:px-4 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition-all text-sm sm:text-base" /></div>
                      <div className="space-y-1.5"><label className="text-xs xs:text-sm font-medium text-gray-700">Phone *</label><input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="+44 7XXX XXX XXX" required className="w-full h-10 xs:h-11 sm:h-12 px-3 xs:px-4 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition-all text-sm sm:text-base" /></div>
                    </div>
                  </div>

                  <div>
                    <div className="flex items-center gap-2 mb-4"><div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center"><span className="text-white font-bold text-xs">2</span></div><h4 className="font-semibold text-gray-900 text-sm xs:text-base">Shipment Details</h4></div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 xs:gap-4">
                      <div className="space-y-1.5"><label className="text-xs xs:text-sm font-medium text-gray-700">Service Required</label>
                        <select name="serviceType" value={formData.serviceType} onChange={handleChange} required className="w-full h-10 xs:h-11 sm:h-12 px-3 xs:px-4 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition-all text-sm sm:text-base appearance-none cursor-pointer">
                      ...
                      <div className="space-y-1.5"><label className="text-xs xs:text-sm font-medium text-gray-700">What Are You Shipping?</label><input type="text" name="cargoDescription" value={formData.cargoDescription} onChange={handleChange} placeholder="e.g. 4 pallets of tiles, 500kg" className="w-full h-10 xs:h-11 sm:h-12 px-3 xs:px-4 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition-all text-sm sm:text-base" /></div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 xs:gap-4 mt-3 xs:mt-4">
                      <div className="space-y-1.5"><label className="text-xs xs:text-sm font-medium text-gray-700">Collection From</label><input type="text" name="pickupLocation" value={formData.pickupLocation} onChange={handleChange} placeholder="e.g. London W7 or full postcode" className="w-full h-10 xs:h-11 sm:h-12 px-3 xs:px-4 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition-all text-sm sm:text-base" /></div>
                      <div className="space-y-1.5"><label className="text-xs xs:text-sm font-medium text-gray-700">Deliver To</label><input type="text" name="deliveryLocation" value={formData.deliveryLocation} onChange={handleChange} placeholder="e.g. Manchester M1 or full postcode" className="w-full h-10 xs:h-11 sm:h-12 px-3 xs:px-4 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition-all text-sm sm:text-base" /></div>
                    </div>
                  </div>

                  <div>
                    <div className="flex items-center gap-2 mb-4"><div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center"><span className="text-white font-bold text-xs">3</span></div><h4 className="font-semibold text-gray-900 text-sm xs:text-base">Additional Info</h4></div>
                    <textarea name="message" value={formData.message} onChange={handleChange} placeholder="Any other details — preferred dates, access restrictions, weight, dimensions, etc." rows={3} className="w-full px-3 xs:px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition-all resize-none text-sm sm:text-base" />
                  </div>

                  {status === "error" && (
                    <div className="p-3 xs:p-4 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm flex items-center gap-2"><div className="w-5 h-5 rounded-full bg-red-100 flex items-center justify-center flex-shrink-0"><span className="text-red-600 text-xs">!</span></div>Something went wrong. Please try again or contact us directly.</div>
                  )}

                  <div className="flex flex-col sm:flex-row gap-3 pt-2">
                    <Button type="submit" disabled={status === "loading"} className="flex-1 h-12 xs:h-14 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm xs:text-base shadow-lg shadow-blue-600/25 hover:shadow-xl transition-all touch-target disabled:opacity-70 rounded-xl group">
                    ...
                    <button type="button" onClick={handleWhatsApp} className="inline-flex items-center justify-center gap-2 h-12 xs:h-14 px-6 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white font-semibold text-sm xs:text-base shadow-lg shadow-green-600/20 transition-all"><MessageCircle className="w-5 h-5" />Send via WhatsApp</button>
                  </div>
          ...
          <div className="flex flex-wrap justify-center gap-4 xs:gap-6 mt-8 xs:mt-10">
            ...
            <div className="flex items-center gap-2 text-gray-600"><Clock className="w-4 h-4 xs:w-5 xs:h-5 text-blue-600" /><span className="text-xs xs:text-sm font-medium">Quoted in under 2 hours</span></div>
...
const CareersSection = () => {
  ...
  return (
    <section id="careers" ref={fade.ref} className={`py-10 xs:py-12 sm:py-16 md:py-20 lg:py-24 bg-gray-900 text-white overflow-hidden relative ${fade.className}`}>
      ...
      <div className="container mx-auto px-3 xs:px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 xs:gap-8 sm:gap-10 lg:gap-12 xl:gap-16 items-start lg:items-center">
          <div className="text-center lg:text-left order-2 lg:order-1">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600/20 rounded-full mb-4 border border-blue-500/20"><Users className="w-4 h-4 text-blue-400" /><span className="text-blue-400 font-semibold text-sm">We're Hiring</span></div>
            <h2 className="font-display text-xl xs:text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-3 xs:mb-4 sm:mb-6 leading-tight">Join the <span className="bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">{company.displayName}</span> Team</h2>
            ...
            <div className="hidden lg:block">
              <Button size="lg" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold h-12 sm:h-14 px-6 sm:px-8 text-sm sm:text-base shadow-lg shadow-blue-600/30 touch-target rounded-xl group" onClick={() => document.getElementById("career-form")?.scrollIntoView({ behavior: "smooth" })}>Apply Now<ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" /></Button>
            </div>
          </div>

          <div id="career-form" className="order-1 lg:order-2 w-full">
            <div className="bg-white rounded-lg xs:rounded-xl sm:rounded-2xl md:rounded-3xl p-3 xs:p-4 sm:p-6 md:p-8 shadow-2xl max-w-md mx-auto lg:max-w-none ring-1 ring-white/10">
              ...
              {status === "success" ? (
                <div className="text-center py-4 xs:py-6 sm:py-8">
                  <div className="w-10 h-10 xs:w-12 xs:h-12 sm:w-16 sm:h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-2 xs:mb-3 sm:mb-4 animate-in zoom-in duration-300"><CheckCircle2 className="w-5 h-5 xs:w-6 xs:h-6 sm:w-8 sm:h-8 text-green-600" /></div>
...
const FloatingWhatsApp = () => {
  ...
  return (
    <a
      ...
      className={`hidden lg:flex fixed bottom-8 right-8 z-50 w-14 h-14 bg-[#25D366] hover:bg-[#20bd5a] rounded-full items-center justify-center shadow-lg shadow-green-600/30 hover:shadow-xl hover:scale-110 transition-all duration-300 ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}
